...
def train_stage_with_early_stopping(
    ...
    for epoch in pbar:
        ...
        if (
            update_grid
            and config.training.update_every > 0
            and epoch <= config.training.stop_update_epoch
            and (epoch == 1 or (epoch - 1) % config.training.update_every == 0)
        ):
            with torch.no_grad():
        ...
        if np.isfinite(old_class_accuracy):
            if old_class_accuracy >= best_old_class_accuracy:
                ...
                old_accuracy_bad_epochs = 0
            elif best_old_class_accuracy - old_class_accuracy > config.training.anti_forgetting_tolerance:
        ...
        should_check = epoch % config.training.check_every == 0 or epoch == epochs
        if should_check:
            if validation_loss < best_loss - config.training.min_delta:
            ...
            if config.training.enabled and steps_without_improvement >= config.training.patience:
                stopped_early = True
        ...
        if (
            config.training.anti_forgetting_enabled
            and old_accuracy_bad_epochs >= config.training.anti_forgetting_patience
        ):
            stopped_early = True
