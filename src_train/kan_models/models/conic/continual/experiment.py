...
from kan_models.common.runtime import configure_matplotlib, detect_device
from kan_models.common.shared import json_number, write_json
from kan_models.models.conic.config import (
    CONTINUAL_CONFIG_VARIANTS,
    DEFAULT_CONIC_CONFIG_PATH,
    ContinualConfig,
    load_continual_config,
)
from kan_models.models.conic.plots.common import plot_kan_edge_functions, save_confusion_matrix, save_final_loss_summary
from kan_models.models.conic.plots.continual import (
    plot_class_test_metrics,
    plot_continual_error,
    plot_continual_losses,
...
def build_run_config(
    ...
    return {
        ...
        "early_stopping": {
            "validation_loss_enabled": config.training.enabled,
            "check_every": config.training.check_every,
            "patience": config.training.patience,
            "min_delta": config.training.min_delta,
            "anti_forgetting_enabled": config.training.anti_forgetting_enabled,
            "anti_forgetting_tolerance": config.training.anti_forgetting_tolerance,
            "anti_forgetting_patience": config.training.anti_forgetting_patience,
        },
        "standardization": {
...
def run_continual(
    config_path: str | Path = DEFAULT_CONIC_CONFIG_PATH,
    variant: str | None = None,
) -> pd.DataFrame:
    """Run the full continual learning experiment from data loading to reports."""
    config = load_continual_config(config_path, variant=variant)
...
def resolve_cli_inputs(
    ...
    """Choose the TOML config path and optional CLI override for the schedule variant."""
    selected_config = Path(config_path) if config_path else Path(default_config)
    selected_variant = None if variant is None else CONTINUAL_CONFIG_VARIANTS.get(variant, variant)
    return selected_config, selected_variant


def main(argv: list[str] | None = None, default_config: str | Path = DEFAULT_CONIC_CONFIG_PATH) -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional CLI override for the schedule variant in the config. "
            "Use 'standard' or 'reversed'."
        ),
    )
    args = parser.parse_args(argv)
    selected_config, selected_variant = resolve_cli_inputs(args.config, args.variant, default_config)
    print(f"Continual config: {selected_config}")
    if selected_variant is not None:
        print(f"Continual variant override: {selected_variant}")
    run_continual(selected_config, variant=selected_variant)
    return 0
