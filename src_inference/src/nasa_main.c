...
static volatile float nasa_benchmark_sink = 0.0f;
...
static void run_warmup(int n) {
    ...
    for (int i = 0; i < KAN_WARMUP_RUNS; ++i) {
        sink += kan_infer_vector(NASA_TEST_FEATURES[i % n]);
    }

    nasa_benchmark_sink = sink;
}
...
static void run_measured_inference(float *predictions, int n) {
    ...
    for (int i = 0; i < n; ++i) {
        predictions[i] = kan_infer_vector(NASA_TEST_FEATURES[i]) * KAN_TARGET_SCALE;
    }
...
int main(int argc, char **argv) {
    ...
    if (NASA_TEST_INPUT_DIM != KAN_INPUT_DIM) {
        fprintf(stderr,
    ...
    printf("measurement = kan_infer_vector loop only\n");
