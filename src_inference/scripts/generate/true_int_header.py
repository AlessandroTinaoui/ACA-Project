#!/usr/bin/env python3
"""Generate include/kan_model_true_int.h for the real integer KAN path."""
...
MODEL_ALIASES = {
    "true_int16": ROOT.parent / "artifacts" / "nasa_kan" / "nasa_kan_quantkan_uniform_w16a16_pc_export.json",
    "true_int8": ROOT.parent / "artifacts" / "nasa_kan" / "nasa_kan_quantkan_uniform_w8a8_pc_export.json",
}
REQUIANT_SHIFT = 24
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate include/kan_model_true_int.h from a quantized KAN export."
    )
...
def main() -> None:
    ...
    for layer_index, layer in enumerate(layers):
        ...
        edge_offsets.append(len(edge_control_mul))
        ...
        for input_index in range(input_size):
            for output_index in range(output_size):
                ...
                cp_q = [quantize_symmetric(value, cp_scale, qmax) for value in cp_real]
                if len(cp_q) != num_control_points:
                    raise ValueError(
                        f"Layer {layer_index}, edge ({input_index}, {output_index}): "
                        f"expected {num_control_points} control points, found {len(cp_q)}."
                    )
                edge_control_points_ioc_q.extend(cp_q)
...
#if defined(__GNUC__)
#define KATI_ALIGN_64 __attribute__((aligned(64)))
#else
#define KATI_ALIGN_64
#endif
...
static const int16_t KATI_EDGE_SCALE_BASE_Q[KATI_NUM_EDGES] KATI_ALIGN_64 = {{
{format_1d_int_array(edge_scale_base_q, "    ")}
}};

static const int64_t KATI_EDGE_SCALE_BASE_MUL[KATI_NUM_EDGES] KATI_ALIGN_64 = {{
