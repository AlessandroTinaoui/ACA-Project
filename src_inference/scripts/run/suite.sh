...
CONFIG_PATH="${1:-src_train/configs/train_config.toml}"
NUM_INPUTS="${2:-0}"
FP32_RUN_NAME="${3:-fp32_full}"
Q16_RUN_NAME="${4:-quant_w16a16_full}"
Q8_RUN_NAME="${5:-quant_w8a8_full}"
TI16_RUN_NAME="${6:-true_int_w16a16_full}"
TI8_RUN_NAME="${7:-true_int_w8a8_full}"
...
echo "[1/8] Train fp32"
"$PYTHON_BIN" src_train/kan_models/nasa/train.py --config "$CONFIG_PATH"

echo "[2/8] Run fp32 on gem5"
bash src_inference/scripts/run/gem5.sh fp32 default "$NUM_INPUTS" "$FP32_RUN_NAME"

echo "[3/8] Quantize w16/a16"
"$PYTHON_BIN" src_train/kan_models/nasa/quantize.py \
  --config "$CONFIG_PATH" \
  --w-bit 16 \
  --a-bit 16 \
  --device cpu

echo "[4/8] Run w16/a16 on gem5"
bash src_inference/scripts/run/gem5.sh quant 16 \
  "../artifacts/nasa_kan/nasa_kan_quantkan_uniform_w16a16_pc_export.json" \
  "$NUM_INPUTS" \
  "$Q16_RUN_NAME"

echo "[5/8] Run true-int16 on gem5"
bash src_inference/scripts/run/gem5.sh true-int 16 \
  "../artifacts/nasa_kan/nasa_kan_quantkan_uniform_w16a16_pc_export.json" \
  "$NUM_INPUTS" \
  "$TI16_RUN_NAME"

echo "[6/8] Quantize w8/a8"
"$PYTHON_BIN" src_train/kan_models/nasa/quantize.py \
  --config "$CONFIG_PATH" \
  --w-bit 8 \
  --a-bit 8 \
...
echo "[7/8] Run w8/a8 on gem5"
bash src_inference/scripts/run/gem5.sh quant 8 \
  "../artifacts/nasa_kan/nasa_kan_quantkan_uniform_w8a8_pc_export.json" \
  "$NUM_INPUTS" \
  "$Q8_RUN_NAME"

echo "[8/8] Run true-int8 on gem5"
bash src_inference/scripts/run/gem5.sh true-int 8 \
  "../artifacts/nasa_kan/nasa_kan_quantkan_uniform_w8a8_pc_export.json" \
  "$NUM_INPUTS" \
  "$TI8_RUN_NAME"
...
COMPARE_OUT="src_inference/simulation_metrics/compare/fp32_vs_quant_full.md"

"$PYTHON_BIN" src_inference/scripts/stats/compare_runs.py \
...
echo
echo "Completed gem5 comparison suite."
